...
def logout(request):
    ...
    return HTTPFound(location = request.route_url('inici'),
                     headers = headers)
# Fi

@view_config(route_name='productes', renderer='productes.mako',permission='usuaris')
def productes_view(request):
   dades=objecteProducte.llegir_arxiu()
   return { "productes":dades, 'logged_in':authenticated_userid(request) }
     
@view_config(route_name='fercomanda',renderer='comanda_feta.mako')
def recollir_dades(request):
    dadesRecollides=objecteProducte.captarProductes(request)
    return {"recollir":dadesRecollides, 'logged_in':authenticated_userid(request)}


@view_config(route_name='inici', renderer='inici.mako')
def inici_view(request):
   return {"Titol":"Inici", 'logged_in':authenticated_userid(request)}

@view_config(route_name='comanda', renderer='comanda.mako',permission='administradors' )
def comanda_view(request):
    dades=objecteProducte.llegirComanda()
    return { "productes":dades, 'logged_in':authenticated_userid(request) }
